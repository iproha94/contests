...
def f_c(x, n):
    set_factor_x = set(factor(x))
    counts = [0] * len(set_factor_x)
    for j in range(1, n + 1):
        factor_j = factor(j)
        for i, px in enumerate(set_factor_x):
            counts[i] += factor_j.count(px)
...
if os.environ.get('DEBUG', False):
    print(f"{f_c(10, 2)} = 2")
    print(f"{f_c(20190929, 1605)} = 363165664")
    print(f"{f_c(947, 987654321987654321)} = 593574252")
else:
